...
if(isset($_POST['submit'])) { // handling form submittin
    ...
    // Validate inputs
    if(empty($restaurant_id) || empty($date) || empty($time) || empty($guests)) {
        $error = "Please fill in all required fields.";
    } else {
        // Use prepare statement to prevent SQL injection
        $stmt = $db->prepare("INSERT INTO table_reservations(user_id, restaurant_id, reservation_date, reservation_time, guests, special_requests, status) 
                             VALUES (?, ?, ?, ?, ?, ?, 'pending')");
        
        if($stmt) { //iissis (Interger , string mix )
            $stmt->bind_param("iissis", $user_id, $restaurant_id, $date, $time, $guests, $special_requests);
            
            if($stmt->execute()) {
                header('location:table_reservation.php');
                exit();
            } else {
                $error = "Error submitting reservation: " . $stmt->error;
            }
            
            $stmt->close();
        } else {
            $error = "Error preparing statement: " . $db->error;
        }
    }
}
?>
...
<head>
    ...
    <title>Add Table Reservation</title>
            ...
            <div class="row">
                <div class="col-md-8 offset-md-2">
                    <div class="reservation-form">
                        <h2 class="text-center mb-4">Add Table Reservation</h2>
